...
def scrape_cinemas(region: Region, host: str) -> list[Cinema] | None:
    ...
    for cinema in _parse_cinema_listings(cinemas_html):
        cinema_details_url = CINEMA_DETAILS_URL_TEMPLATE.format(host=host, cinema_slug=cinema['slug'])
        cinema_details_html = web_utils.fetch_html(cinema_details_url)
        cinema_enriched = _enrich_cinema_with_url(cinema['name'], region.name, cinema_details_html)
...
def _enrich_cinema_with_url(cinema_name: str, region_name: str, html: str) -> Cinema:
    ...
    return Cinema(id=str(uuid4()), name=cinema_name, homepage_url=cinema_url, region=region_name)
